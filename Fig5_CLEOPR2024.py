...
checkpoint = torch.load("Data_Fig5_diff/net_diff_quarternoise_f{}_train{}.pt".format(f,train_optical))
...
for train_optical in [False, True]:
    ...
    np.savez("Data_Fig5_diffraction/Iminmax_realIR_{}.npz".format(train_optical), 
             Imin=I_min.cpu().numpy(), Imax=I_max.cpu().numpy())


test_accuracy = np.array(test_accuracy).reshape(2, len(dark_noise_level), -1)

test_noisy_signal = np.array(test_noisy_signal).reshape(-1, *I_out.shape)
test_pure_signal = np.array(test_pure_signal).reshape(-1, *I_out.shape)
...
#%%
# dx_ir = 0.75
# resol_ir = image.shape[0]
# W_ir = dx_ir * resol_ir
# f = f

# x_ir = np.arange(-W_ir/2, W_ir/2, dx_ir) + dx_ir/2

# Y_ir, X_ir = np.meshgrid(x_ir, x_ir)

# Phi_l = -np.pi*(X_ir**2+Y_ir**2)/ f
# Phi_ml1 = -np.pi*((X_ir-center1[0])**2/focal_length1[0] + (Y_ir-center1[1])**2/focal_length1[1])
# Phi_ml2 = +np.pi*((X_ir-center2[0])**2/focal_length2[0] + (Y_ir-center2[1])**2/focal_length2[1])
# Phi_ml3 = -np.pi*((X_ir-center3[0])**2/focal_length3[0] + (Y_ir-center3[1])**2/focal_length3[1])



# Prop = om.prop_matrix(resol_ir, resol_ir, dx_ir, f, 1.0, Ndx=2).numpy()

# Green = Prop @ np.diag(Phi_l.reshape(-1)) @ Prop @ Prop @ np.diag(Phi_l.reshape(-1)) @ Prop


# #%% Supp fig: concatenated images


# pure_concat = np.transpose(test_pure_signal.reshape(2,10,28,28), axes=(0,3,1,2))[:,::-1,:,::-1].reshape(-1,10,28).reshape(56,-1)
# noisy_concat = np.transpose(test_noisy_signal.reshape(2,10,28,28), axes=(0,3,1,2))[:,::-1,:,::-1].reshape(-1,10,28).reshape(56,-1)

# fig, ax = plt.subplots(3,1, figsize=(6.5, 4), sharex=True,sharey=True)

# is0 = ax[0].imshow(pure_concat, cmap=plt.cm.inferno)
# ax[0].set_title("Pure images", fontsize=7)
# ax[0].set(xticks=14+np.arange(10)*28, xticklabels=np.arange(10), yticks=(14,42), yticklabels=(r"$4f$", "Meta"))

# I_dark = I_diff/4
# gaussian = np.random.normal(loc=0, scale=I_dark, size=pure_concat.shape)
# is1 = ax[1].imshow(pure_concat+gaussian, cmap=plt.cm.cividis)
# ax[1].set_title(r"Noisy images with $\sigma_\mathrm{dark}=\Delta I/4$", fontsize=7)

# I_dark = I_diff/2
# gaussian = np.random.normal(loc=0, scale=I_dark, size=pure_concat.shape)
# is2 = ax[2].imshow(pure_concat+gaussian, cmap=plt.cm.cividis)
# ax[2].set_title(r"Noisy images with $\sigma_\mathrm{dark}=\Delta I/2$", fontsize=7)

# axins0 = ax[0].inset_axes([1.01, 0.25, 0.02, 0.5])
# cb0 = plt.colorbar(is0, orientation='vertical', cax=axins0, extend=None)
# axins1 = ax[1].inset_axes([1.01, 0.25, 0.02, 0.5])
# cb1 = plt.colorbar(is1, orientation='vertical', cax=axins1, extend=None)
# axins2 = ax[2].inset_axes([1.01, 0.25, 0.02, 0.5])
# cb2 = plt.colorbar(is2, orientation='vertical', cax=axins2, extend=None)

# fig.tight_layout()
# time = datetime.now().strftime("%Y%m%d%H%M")[2:]
# fig.savefig("Figures/SuppFig_metaimages_{}.pdf".format(time), dpi=900, transparent=True)


#%% CLEO-PR image


fig, ax = plt.subplots(3,3, figsize=(5,3.6))
ax[0,0].pcolormesh(Xl,Yl, np.angle(np.exp(1j*(phase1-lens1))).reshape(resol_lens,-1), vmin=-np.pi, vmax=np.pi, cmap=plt.cm.twilight, linewidth=0,rasterized=True)
ax[0,1].pcolormesh(Xs,Ys, np.angle(np.exp(1j*phase2)).reshape(resol_scene,-1), vmin=-np.pi, vmax=np.pi, cmap=plt.cm.twilight, linewidth=0,rasterized=True)
pc02 = ax[0,2].pcolormesh(Xl,Yl, np.angle(np.exp(1j*(phase3-lens3))).reshape(resol_lens,-1), vmin=-np.pi, vmax=np.pi, cmap=plt.cm.twilight, linewidth=0,rasterized=True)

axins = ax[0,2].inset_axes([1.05, 0.25, 0.05, 0.5])
cb = plt.colorbar(pc02, orientation='vertical', cax=axins, extend=None)
cb.ax.set_yticks([-np.pi, 0, np.pi])
cb.ax.set_yticklabels([r'$-\pi$', 0, r'$\pi$'])

digit = 1
vmin, vmax = test_pure_signal[:,digit].min(), test_pure_signal[:,digit].max()
ax[1,0].pcolormesh(Xi,Yi, test_pure_signal[0,digit].reshape(28,-1), cmap=plt.cm.magma, vmin=vmin,vmax=vmax, linewidth=0,rasterized=True)
pc20 = ax[2,0].pcolormesh(Xi,Yi, test_pure_signal[1,digit].reshape(28,-1), cmap=plt.cm.magma, vmin=vmin,vmax=vmax, linewidth=0,rasterized=True)


axins = ax[2,0].inset_axes([1.05, 0.25, 0.05, 0.5])
cb = plt.colorbar(pc20, orientation='vertical', cax=axins, extend='min')



noise = np.random.normal(loc=0, scale=I_diff/6, size=(2,784))
ax[1,1].pcolormesh(Xi,Yi, (test_pure_signal[0,digit]+noise[0]).reshape(28,-1), cmap=plt.cm.bone, linewidth=0,rasterized=True)
ax[2,1].pcolormesh(Xi,Yi, (test_pure_signal[1,digit]+noise[0]).reshape(28,-1), cmap=plt.cm.bone, linewidth=0,rasterized=True)

noise = np.random.normal(loc=0, scale=I_diff/3, size=(2,784))
ax[1,2].pcolormesh(Xi,Yi, (test_pure_signal[0,digit]+noise[0]).reshape(28,-1), cmap=plt.cm.bone, linewidth=0,rasterized=True)
pc22 = ax[2,2].pcolormesh(Xi,Yi, (test_pure_signal[1,digit]+noise[0]).reshape(28,-1), cmap=plt.cm.bone, linewidth=0,rasterized=True)

axins = ax[2,2].inset_axes([1.05, 0.25, 0.05, 0.5])
cb = plt.colorbar(pc22, orientation='vertical', cax=axins, extend=None)
cb.ax.set_yticks([(test_pure_signal[1,digit]+noise[0]).min(), (test_pure_signal[1,digit]+noise[0]).max()])
cb.ax.set_yticklabels(['min', 'max'])


ax[1,0].set_ylabel(r'$y/\lambda$')
ax[2,1].set_xlabel(r'$x/\lambda$')

fig.tight_layout()
time = datetime.now().strftime("%Y%m%d%H%M")[2:]
fig.savefig("Figures/CLEOPR_{}.pdf".format(time), dpi=900, transparent=True)
...
# # max_pos = np.unravel_index(np.argmax(test_pure_signal[0,digit]), (28,28))
# # ax[0,3].plot(Xi[max_pos[0],max_pos[1]], Yi[max_pos[0],max_pos[1]], 'x')
